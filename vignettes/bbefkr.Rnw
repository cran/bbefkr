...
# q=2 represents the semi-metric based on the second derivative, since functions can be
...
R> np_global <- bayMCMC_np_global(data_x=simcurve_smooth_normerr, data_y=simresp_np_normerr,
  	data_xnew=simcurve_smooth_normerr, warm=1000, M=1000, range.grid=c(0,pi), 
		q=2, nknot=20)
...
# it measures how many iterations are required to have the iid draws from the posterior
...
\begin{Verbatim}[fontsize=\small]
# error density is approximated by a kernel density of residuals with a global bandwidth
# using the semi-metric based on the second derivative
...
# a global bandwidth using the semi-metric based on three retained principal components
...
# localised bandwidths using the semi-metric based on three retained principal components
...
# we can then predict its response using a functional nonparametric regression
R> fat_np_global <- bayMCMC_np_global(data_x = specurves[1:44,], data_y = fat[1:44], 
...
R> fat_np_local <- bayMCMC_np_local(data_x = specurves[1:44,], data_y = fat[1:44], 
			data_xnew = specurves[45,], range.grid=c(0,pi), q=2, nknot=20)
...
\begin{Verbatim}[fontsize=\small]
R> fat_np_global_d1 <- bayMCMC_np_global(data_x = specurves[1:44,], data_y = fat[1:44], 
  		data_xnew = specurves[45,], range.grid=c(0,pi), q=1, nknot=20)
                                   
R> fat_np_global_d2 <- bayMCMC_np_global(data_x = specurves[1:44,], data_y = fat[1:44], 
			data_xnew = specurves[45,], range.grid=c(0,pi), q=2, nknot=20)                                   
                                   
R> fat_np_global_pca <- bayMCMC_np_global(data_x = specurves[1:44,], data_y = fat[1:44], 
			data_xnew = specurves[45,], semimetric="pca", q=3)

R> c(fat_np_global_d1$mlikeres, fat_np_global_d2$mlikeres, fat_np_global_pca$mlikeres)
\end{Verbatim}
